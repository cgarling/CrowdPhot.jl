...
function estimate_initial_star_params(star::EmpiricalStar{T}, image) where {T}
    ...
    return ConstructionBase.setproperties(star, (bkg = T(bkg), flux = T(flux)))
end
...
function fit_star_against_epsf(
    ...
    if all(isfinite, p) && p[3] > eps(T)
        ...
        return ConstructionBase.setproperties(star, (used = false, converged = false, cost = T(Inf)))
    end
end
...
function remove_centroid_drift(stars, old_centroids)
    ...
    if !isempty(dxs)
        ...
        for k in eachindex(stars)
            stars[k].used || continue
            stars[k] = ConstructionBase.setproperties(stars[k], (x = stars[k].x - dx_med, y = stars[k].y - dy_med))
        end
    end
    return stars
end
